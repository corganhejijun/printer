...
static int _cyBlankMargin=4,_cygMargin=1;
...
#define CX_GRIPPER  3
#define CY_GRIPPER  3
#define CX_BORDER_GRIPPER 1
#define CY_BORDER_GRIPPER 1
#define CX_GRIPPER_GAP	1
#define CY_GRIPPER_GAP	1
#define CX_GRIPPER_ALL CX_GRIPPER*2 + CX_BORDER_GRIPPER*2+CX_GRIPPER_GAP
#define CY_GRIPPER_ALL CY_GRIPPER*2 + CY_BORDER_GRIPPER*2+CY_GRIPPER_GAP
...
void CMenuBar::OnSysCommand(UINT nID, LPARAM lParam) 
{
	nID=nID;
	return;
	if (IsValidIndex(0) &&	(m_arrItem[0]->GetStyle() & MISTYLE_TRACKABLE)) {
		TrackPopup(0);
		return;		// eat it!
	}
/*	if(m_bSysMenuIcon)
...
void CMenuBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if(IsValidIndex(nIndex)){
		if(nIndex==0&&g_pMenuBar==NULL&&m_bIgnoreClick){
			m_bIgnoreClick=FALSE;
			CFrameWnd*pChild=GetTopLevelFrame()->GetActiveFrame();
			if(pChild==NULL)return;
			UINT id=pChild->GetSystemMenu(FALSE)->GetDefaultItem(0);
			if(id==-1)return;
			pChild->PostMessage(WM_SYSCOMMAND,id,NULL);
			return;
		}
		OnLButtonDown(nFlags,point);
	}else	CControlBar::OnLButtonDblClk(nFlags, point); 
}
		static int count=0;
...
CSize CMenuBar::CalcSize(int nCount)
{
	...
	sizeResult.cy+=_cyBlankMargin*2;
	return sizeResult;
}
...
CSize CMenuBar::CalcVertDockSize(int nCount)
{
	...
	sizeResult.cx = _cyMenuOnBar+_cyBlankMargin*2;
	return sizeResult;
}
...
void CMenuButton::InitAccessKeyAndVerticalLinePoint()
{
	...
	if (nIndex == -1) {
		...
		m_ptLineFrom = CPoint(0, CXTEXTMARGIN);
		m_ptLineTo	 = CPoint(0, CXTEXTMARGIN + _CalcTextWidth(m_strBtn.Left(nIndex+2)));
	}
	else {
		m_ptLineFrom = CPoint(0, CXTEXTMARGIN + _CalcTextWidth(m_strBtn.Left(nIndex)));
		m_ptLineTo = CPoint(0/*_cyTextMargin*/, CXTEXTMARGIN + _CalcTextWidth(m_strBtn.Left(nIndex+2)));
	}
}
...
//	pWndSentCmd->SendMessage(WM_ENTERMENULOOP,FALSE);
}
...
//	m_hDocIcon = (HICON)::GetClassLong(hWndChild, GCL_HICONSM);
	if(pWndChild)m_hDocIcon = pWndChild->GetIcon(TRUE);
	else m_hDocIcon=NULL;
...
//	::TrackPopupMenuEx(hSysMenu, fuFlags, pt.x, pt.y, hWndChild, &tpm);
	::TrackPopupMenuEx(hSysMenu, fuFlags, pt.x, pt.y, pWndSentCmd->GetSafeHwnd(), &tpm);
	::SendMessage(hWndChild,WM_ENTERMENULOOP,FALSE,0);
}
...
void CMenuBar::_DrawGripper(CDC* pDC, const CRect& rect)
{
	...
	if (bDrawGripper)
	{
		...
		if (m_dwStyle & CBRS_ORIENT_HORZ)
		{
			if (bDrawGripper) {
				pDC->Draw3dRect(rect.left+CX_BORDER_GRIPPER,
					rect.top+m_cyTopBorder+_cygMargin,
					CX_GRIPPER, rect.Height()-m_cyTopBorder-m_cyBottomBorder-2*_cygMargin,
					clrBtnHilite, clrBtnShadow);
				pDC->Draw3dRect(rect.left+CX_BORDER_GRIPPER+CX_GRIPPER+CX_GRIPPER_GAP,
					rect.top+m_cyTopBorder+_cygMargin,
					CX_GRIPPER, rect.Height()-m_cyTopBorder-m_cyBottomBorder-2*_cygMargin,
					clrBtnHilite, clrBtnShadow);		
			}		
		...
		{
			if (bDrawGripper) {
				pDC->Draw3dRect(rect.left+m_cyTopBorder+_cygMargin,
					rect.top+CY_BORDER_GRIPPER,
					rect.Width()-m_cyTopBorder-m_cyBottomBorder-2*_cygMargin, CY_GRIPPER,
					clrBtnHilite, clrBtnShadow);
				pDC->Draw3dRect(rect.left+m_cyTopBorder+_cygMargin,
					rect.top+CY_BORDER_GRIPPER+CY_GRIPPER+CY_GRIPPER_GAP,
					rect.Width()-m_cyTopBorder-m_cyBottomBorder-2*_cygMargin, CY_GRIPPER,
					clrBtnHilite, clrBtnShadow);
			}
		}
